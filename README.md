...
npm run db:seed       # демо-пользователь и данные за 4 месяца
...
2. Войдите демо-аккаунтом или зарегистрируйтесь:

| | |
|---|---|
| **Демо email** | `demo@family.local` (опечатка `demo@femily.local` тоже сработает) |
| **Демо пароль** | `demo1234` |

Демо-данные: операции за **февраль–май 2026**, лимиты по категориям, повторяющиеся платежи.

Заполнить БД тестовыми данными вручную:

```bash
cd backend && npm run db:seed
```

3. После входа: главная, транзакции (редактирование по иконке карандаша), отчёты, **Настройки** (семья, лимиты, повторяющиеся)
...
При старте backend автоматически применяет миграции и заполняет БД демо-данными (`prisma db seed`).
...
| `npm run db:seed` | Демо-данные (4 месяца истории) |
